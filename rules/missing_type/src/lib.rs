...
use rustc_hir::{Body, Expr, ExprKind, LetStmt, LocalSource, PatKind};
...
impl<'tcx> LateLintPass<'tcx> for MissingType {
    ...
    fn check_local(
        ...
        // Ignore non-normal let statements, such as those generated by the
        // compiler for desugared constructs, as they may not have an explicit
        // type annotation even if the original source code does. This
        // check ensures that the lint only applies to normal let statements
        // written by the user, and not to those generated by the compiler
        // during desugaring.
